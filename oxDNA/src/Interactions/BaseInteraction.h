...
map<int, number> BaseInteraction<number, child>::get_system_energy_split(BaseParticle<number> **particles, int N, BaseList<number> *lists) {
	OX_LOG(Logger::LOG_INFO, "BaseInteraction<number, child>::get_system_energy_split");

	std::map<int, number> energy_map;
	OX_LOG(Logger::LOG_INFO, "std::map<int, number> energy_map;");
	...
	OX_LOG(Logger::LOG_INFO, "for (int i = 0; i < N; i ++) { end");
