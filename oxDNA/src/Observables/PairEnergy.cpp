...
std::string PairEnergy<number>::get_output_string(llint curr_step) {
	...
	OX_LOG(Logger::LOG_INFO,"setting end: number total_energy_diff = 0.;");
	//bug occured
	...
	OX_LOG(Logger::LOG_INFO, "std::map<int, number>・・・");
	if( _print_header){
		OX_LOG(Logger::LOG_INFO, "start if");
	...
	if(_print_all_particles) {
		...
		OX_LOG(Logger::LOG_INFO,"for(int i = 0; i < (int)・・・");
