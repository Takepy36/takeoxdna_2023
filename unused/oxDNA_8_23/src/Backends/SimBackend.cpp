...
void SimBackend<number>::_print_ready_observables(llint curr_step) {
	llint total_bytes = 0;
	typename vector<ObservableOutput<number> *>::iterator it;
	OX_LOG(Logger::LOG_INFO, "SimBackend<number>::_print_ready_observables start (A.T　debag)");
	for(it = _obs_outputs.begin(); it != _obs_outputs.end(); it++) {
		OX_LOG(Logger::LOG_INFO, "SimBackend<number>::_print_ready_observables for (A.T　debag)");
		if((*it)->is_ready(curr_step)) {
			OX_LOG(Logger::LOG_INFO, "SimBackend<number>::_print_ready_observables if (A.T　debag)");
			(*it)->print_output(curr_step);
		}
		OX_LOG(Logger::LOG_INFO, "SimBackend<number>::_print_ready_observables after if (A.T　debag)");
		total_bytes += (*it)->get_bytes_written();
	}
	OX_LOG(Logger::LOG_INFO, "SimBackend<number>::_print_ready_observables after for (A.T　debag)");
	...
	OX_LOG(Logger::LOG_INFO, "SimBackend<number>::_print_ready_observables last (A.T　debag)");
...
void SimBackend<number>::print_observables(llint curr_step) {
	...
	OX_LOG(Logger::LOG_INFO, "SimBackend<number>::print_observables bool someone_ready (A.T　debag)");
	typename vector<ObservableOutput<number> *>::iterator it;
	for(it = _obs_outputs.begin(); it != _obs_outputs.end(); it++) {
		OX_LOG(Logger::LOG_INFO, "SimBackend<number>::print_observables for (A.T　debag)");
		if((*it)->is_ready(curr_step)) someone_ready = true;
	}
	OX_LOG(Logger::LOG_INFO, "SimBackend<number>::print_observables after for (A.T　debag)");
	if(someone_ready) _print_ready_observables(curr_step);//ここで実行停止
	OX_LOG(Logger::LOG_INFO, "SimBackend<number>::print_observables last (A.T　debag)");
	_backend_info = std::string ("");
}
