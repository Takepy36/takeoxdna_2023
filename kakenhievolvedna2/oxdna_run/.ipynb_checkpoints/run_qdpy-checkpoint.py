...
def run_qdpy(source_dirpath,output_dirpath,bgt=10000,plot=True):
    # Create container and algorithm. Here we use MAP-Elites, by illuminating a Grid container by evolution.
    #コンテナとアルゴリズムを作成する。ここでは、進化によってGridコンテナを照らすことで、MAP-Elitesを使用しています。

    #評価結果を配置するgridを作成。
    if os.path.basename(output_dirpath) == "loop0":
        #出力先output_dirpath = loop0の時だけ
        ...
        grid = use_pickle.read_pickle(source_dirpath+"/qdpy_grid.p")
    ...
    print("run_qdpy output dir:",output_dirpath)
    ...
    algo.optimise(functools.partial(eval_from_svm,svm=SVRs,th=0.95))
    # print(algo.summary())
    ...
    #for debag
    use_pickle.dump_to_pickle(output_dirpath,[grid],["qdpy_grid"])
...
# run_qdpy("20230731_0000",
#         "20230731_0000/20230811_1810/loop0")
