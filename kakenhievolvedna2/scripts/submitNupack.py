...
#def launch_nupack(genome, domains=[submitPepperCorn.domaina, submitPepperCorn.domainb], length=4, prefix="foobar")
def launch_nupack(genome, domains, length, prefix, base_dir, max_complex_size = 3, temperature = 20., default_conc = 1.0e-7, compute_hist = True, timeout=None):
    # Create base directory
    pathlib.Path(base_dir).mkdir(parents=True, exist_ok=True)

    #print(f"@@@@@@@ {[d.sequence for d in domains]}")

    # Prepare nupack input files
    seq_list = getSequenceListFromGenotype(genome, domains, length)
    n = len(seq_list)
    with open(os.path.join(base_dir, prefix + ".con"), "w") as f:
        f.write("\n".join([f"{default_conc}" for _ in range(n)]) + "\n") 
    with open(os.path.join(base_dir, prefix + ".in"), "w") as f:
        f.write(f"{n}\n" + "\n".join(seq_list) + f"\n{max_complex_size}\n")

    with open(os.path.join(base_dir, prefix + ".log"), "w") as f_log:
        # Launch nupack: "complexes" command
        rc_complexes = subprocess.call(["complexes", "-T", f"{temperature}", "-material", "dna", "-pairs", "-mfe", "-degenerate", os.path.join(base_dir, prefix)], stdout=f_log, stderr=f_log, timeout=timeout)
        # Launch nupack: "concentrations" command
        rc_concentrations = subprocess.call(["concentrations", "-pairs", os.path.join(base_dir, prefix)], stdout=f_log, stderr=f_log, timeout=timeout)

    # Compute histogram of structure size
    if compute_hist:
        eq_file = os.path.join(base_dir, prefix + ".eq")
        conc_df = nupackHistogramReader(eq_file, n)
        conc_df.to_pickle(os.path.join(base_dir, prefix + ".p"))
...
if __name__ == "__main__":
    ...
    def _launch(*args, **kwds):
        global pool
        pool.apply_async(launch_nupack, args=args, kwds=kwds)
