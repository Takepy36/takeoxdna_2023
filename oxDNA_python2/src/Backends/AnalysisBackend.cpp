...
void AnalysisBackend::analyse() {
	...
	OX_LOG(Logger::LOG_INFO, "AnalysisBackend::analyse() (%d) started (A.T　debag)", _n_conf);
	//ここで実行停止
	if(_n_conf % 100 == 0 && _n_conf > 0) OX_LOG(Logger::LOG_INFO, "Analysed %d configurations", _n_conf);
	SimBackend<double>::print_observables(_read_conf_step);//問題箇所
	OX_LOG(Logger::LOG_INFO, "simBackend<double>::print_observables (A.T　debag)");
