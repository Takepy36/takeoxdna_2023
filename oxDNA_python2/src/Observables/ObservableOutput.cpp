...
void ObservableOutput<number>::print_output(llint step) {
	OX_LOG(Logger::LOG_INFO, "ObservableOutput<number>::print_output start (A.T　debag)");
	stringstream ss;
	typename vector<BaseObservable<number> *>::iterator it;
	for(it = _obss.begin(); it != _obss.end(); it++) {
		OX_LOG(Logger::LOG_INFO, "ObservableOutput<number>::print_output for (A.T　debag)");
		if(it != _obss.begin()) ss << " ";
		OX_LOG(Logger::LOG_INFO, "ObservableOutput<number>::if(it != _obss.begin()) ss %s",(*it)->name);
		ss << (*it)->get_output_string(step);//何かがあった？
	}

	if(_update_name_with_time) {
		OX_LOG(Logger::LOG_INFO, "ObservableOutput<number>::print_output if (A.T　debag)");
	...
	OX_LOG(Logger::LOG_INFO, "ObservableOutput<number>::print_output after if (A.T　debag)");
	...
	*_output << towrite;
	_output->flush();
	OX_LOG(Logger::LOG_INFO, "ObservableOutput<number>::print_output _output->flush() (A.T　debag)");
