...
std::string AverageEnergy<number>::get_output_string(llint curr_step) {
	OX_LOG(Logger::LOG_INFO, "get_output_string start");
	...
	OX_LOG(Logger::LOG_INFO, "ObservableOutput<number>::BaseParticle<number>;");
	BaseParticle<number> *p;
	BaseParticle<number> *q;
	OX_LOG(Logger::LOG_INFO, "ObservableOutput<number>::BaseParticle<number> *q;");
	number total_energy = 0;
	for(int i = 0; i < (int) neighbour_pairs.size(); i++) {
		OX_LOG(Logger::LOG_INFO, "for(int i = 0; i < (int) neighbour_pairs.size(); i++) {");
	...
	OX_LOG(Logger::LOG_INFO, "for(int i = 0; i < (int) neighbour_pairs.size(); i++) { end");
